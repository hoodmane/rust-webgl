...
use crate::vector::{JsPoint, SliceVec2T};
...
impl DefaultShader {
    ...
    pub fn draw<T : SliceVec2T>(&mut self, transform : Transform, vertices : T, primitive : u32) -> Result<(), JsValue> {
    ...
    // pub fn draw_elements(&mut self, transform : Transform, vertices : &Vec<Point>, primitive : u32) -> Result<(), JsValue> {
    //     self.shader.use_program();
    //     self.geometry.num_vertices = vertices.len() as i32;
    //     self.geometry.num_instances = 1;
    ...
    pub fn get_raster<T : SliceVec2T + Copy>(&mut self, transform : Transform, vertices : T, primitive : u32, target : &mut Buffer) -> Result<(Vec<u8>, i32, i32), JsValue> 
        where <T as IntoIterator>::Item : Into<JsPoint>
    {
        ...
        for v in vertices {
            bounding_box_builder.include(v.into().into());
        }
        ...
        let transform = transform.pre_translate(vector( - bounding_box.left(), - bounding_box.top()));
